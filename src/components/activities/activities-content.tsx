...
export function ActivitiesContent() {
  const { stepByStepMode, simplifiedLanguage, learningPace } = useAccessibilityPrefs();
  ...
  async function submit(answers: { selectedId?: string; order?: string[] }) {
    ...
    setSubmitting(true);
    setSubmitError(null);
    if (answers.selectedId) setSelected(answers.selectedId);
    try {
  ...
  if (!activity) {
    return (
      <div className="mx-auto max-w-lg text-center space-y-4 py-12">
        <h1 className="font-heading text-3xl font-bold">Práctica</h1>
        {loadError ? (
          <>
    ...
    <div className="mx-auto max-w-2xl space-y-8 pb-8">
      ...
      <Card>
        <CardHeader>
          ...
          {activity.type === "CASE_STUDY" && activity.scenario && (
            <p className="rounded-2xl bg-brand-light-bg px-4 py-3 text-sm leading-relaxed text-brand-muted-gray">
              {activity.scenario}
            </p>
          )}
          {simplifiedLanguage && (
            ...
            activity.options?.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex w-full items-center gap-3 rounded-2xl border px-4 py-4 text-left text-sm transition-colors duration-150",
                    !showResult &&
                      "hover:border-brand-cobalt/30 hover:bg-brand-champagne/30 active:scale-[0.99]",
                ...
                >
                  ...
                  {submitting && isSelected && !showResult && (
                    <Loader2 className="ml-auto h-5 w-5 animate-spin text-brand-cobalt" />
                  )}
                  {showCorrect && (
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    {activity.moduleSlug && (
                      <Button size="sm" variant="outline" asChild>
                        <Link href={`/modules/${activity.moduleSlug}`}>
                          Volver al módulo
                        </Link>
                      </Button>
                    )}
